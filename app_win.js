...
const dockerCMD = `cd TEMP_DIR_NAME && docker run --rm -i --net=none -v ${currentDir}\\TEMP_DIR_NAME:/data "${dockerImageName}" /bin/sh -c "${latexCMD} && ${dvisvgmCMD}"`;
    ...
    if (req.body.latexInput) {
        if (validScales.includes(req.body.outputScale)) {
            if (validFormats.includes(req.body.outputFormat)) {
                ...
                fs.writeFileSync(`${tempDirRoot}\\equation.tex`, document); // Write generated .tex file
                
                let result = {};
                
                let finalDockerCMD = dockerCMD.replace('TEMP_DIR_NAME', `${tempDirRoot}`);
                finalDockerCMD = finalDockerCMD.replace('TEMP_DIR_NAME', `${tempDirRoot}`);
                ...
                shell.exec(finalDockerCMD, {async: true}, function() {
                    ...
                    if (fs.existsSync(`${tempDirRoot}\\equation.svg`)) {
                        if (fileFormat === 'svg') { // Converting to SVG, no further processing required
                            ...
                            let finalSvgToImageCMD = svgToImageCMD.replace('SVG_FILE_NAME', `${tempDirRoot}\\equation.svg`);
                            finalSvgToImageCMD = finalSvgToImageCMD.replace('OUT_FILE_NAME', `${tempDirRoot}\\equation.${fileFormat}`);
